...
const STATUS_OPTIONS = ['all', 'pending', 'assigned', 'picking_up', 'delivering', 'completed', 'cancelled'];
                ...
                <div className="flex gap-2 flex-wrap">
                    {STATUS_OPTIONS.map((s) => (
                        <button
                            ...
                            className={`px-3 py-2 text-xs rounded-lg capitalize transition-all ${status === s
                    ...
                    <table className="w-full">
                        <thead>
                            <tr className="border-b border-gray-800">
                                {['Order Code', 'Customer', 'Driver', 'Status', 'Service', 'Total', 'Date'].map((h) => (
                        ...
                        <tbody>
                            {isLoading ? (
                                ...
                                orders.map((order: any) => (
                                    ...
                                    >
                                        <td className="px-6 py-4 text-sm font-mono text-blue-400">{order.orderCode}</td>
                                        <td className="px-6 py-4">
                                            <p className="text-sm">{order.customer?.name}</p>
                                            <p className="text-xs text-gray-500">{order.customer?.phone}</p>
                                        </td>
                                        <td className="px-6 py-4 text-sm text-gray-300">{order.driver?.name || '—'}</td>
                                        <td className="px-6 py-4">
                                            <span className={`px-2.5 py-1 rounded-lg text-xs font-medium capitalize ${STATUS_COLORS[order.status] || ''}`}>
                                                {order.status?.replace('_', ' ')}
                                            </span>
                                        </td>
                                        <td className="px-6 py-4 text-sm capitalize text-gray-300">{order.serviceType}</td>
                                        <td className="px-6 py-4 text-sm font-medium">{order.pricingBreakdown?.total?.toLocaleString()}đ</td>
