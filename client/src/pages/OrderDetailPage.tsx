...
export default function OrderDetailPage() {
    ...
    const complaintMutation = useMutation({
        mutationFn: (data: any) => ordersApi.updateComplaint(id!, data),
        onSuccess: () => {
    ...
    return (
        <div>
            <button onClick={() => navigate('/orders')} className="text-gray-400 hover:text-gray-200 text-sm mb-6 flex items-center gap-1">
                ← Back to Orders
            </button>

            <div className="flex items-center gap-4 mb-8">
                <h1 className="text-2xl font-bold font-mono">{order.orderCode}</h1>
                <span className={`px-3 py-1 rounded-lg text-sm font-medium capitalize border ${STATUS_COLORS[order.status]}`}>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div className="bg-gray-900/80 backdrop-blur border border-gray-800 rounded-2xl p-6">
                        ...
                        <div className="grid grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <p className="text-sm font-medium">{order.driver?.name || 'Unassigned'}</p>
                                <p className="text-sm text-gray-400">{order.driver?.phone || ''}</p>
                            </div>
                            <div>
                    ...
                    <div className="bg-gray-900/80 backdrop-blur border border-gray-800 rounded-2xl p-6">
                        ...
                        <div className="space-y-2">
                            {[
                                ...
                                ['Peak Hour Surcharge', order.pricingBreakdown?.peakHourSurcharge],
                                ['Bulky Item Surcharge', order.pricingBreakdown?.bulkyItemSurcharge],
                            ...
                            <div className="border-t border-gray-700 pt-2 flex justify-between text-sm font-semibold">
                                <span>Total</span>
                                <span className="text-blue-400">{order.pricingBreakdown?.total?.toLocaleString()}đ</span>
                            </div>
                            {order.codAmount > 0 && (
                                <div className="flex justify-between text-sm mt-2">
                                    <span className="text-gray-400">COD Amount</span>
                                    <span className="text-yellow-400">{order.codAmount?.toLocaleString()}đ</span>
                                </div>
                            )}
                        </div>
                    </div>
                        ...
                        <div className="bg-gray-900/80 backdrop-blur border border-gray-800 rounded-2xl p-6">
                            ...
                            <div className="flex gap-3 flex-wrap">
                                {order.deliveryProofImages.map((img: string, i: number) => (
                                    <img key={i} src={img} alt="Delivery proof" className="w-40 h-30 object-cover rounded-xl border border-gray-700" />
                                ))}
                            </div>
                        </div>
                    )}
                ...
                <div className="space-y-6">
                            ...
                            {order.events?.map((event: any, i: number) => (
                                <div key={i} className="flex gap-3">
                                    <div className="flex flex-col items-center">
                                        ...
                                        {i < order.events.length - 1 && <div className="w-px h-full bg-gray-700 mt-1" />}
                                    </div>
                                    <div className="pb-4">
                    ...
                    <div className="bg-gray-900/80 backdrop-blur border border-gray-800 rounded-2xl p-6">
                        ...
                        {order.complaint && (
                            <div className="mb-4">
                                <span className={`px-2.5 py-1 rounded-lg text-xs font-medium capitalize ${STATUS_COLORS[order.complaint.status] || 'bg-gray-700 text-gray-300'}`}>
                                    {order.complaint.status?.replace('_', ' ')}
                                </span>
                                {order.complaint.notes?.map((note: string, i: number) => (
                                    <p key={i} className="text-sm text-gray-400 mt-2">• {note}</p>
                                ))}
                            </div>
                        )}
                        <div className="space-y-3">
                            ...
                            />
                            <button
                                onClick={() => complaintMutation.mutate({ status: complaintStatus, note: complaintNote })}
                        ...
                        <div className="bg-gray-900/80 backdrop-blur border border-gray-800 rounded-2xl p-6">
                            ...
                            <div className="space-y-3">
                                {auditLogs.map((log: any, i: number) => (
                                    <div key={i} className="text-sm border-b border-gray-800 pb-3 last:border-0">
                                        <p className="text-gray-300 font-medium">{log.action}</p>
                                        {log.field && <p className="text-xs text-gray-500">{log.field}: {String(log.oldValue)} → {String(log.newValue)}</p>}
                                        <p className="text-xs text-gray-600">{new Date(log.performedAt).toLocaleString()}</p>
                                    </div>
                                ))}
                            </div>
                        </div>
