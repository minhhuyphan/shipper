...
export class PricingService {
  ...
  async simulate(input: {
    ...
    if (!service || !service.enabled) {
      throw new AppError(
        `Service type "${input.serviceType}" is disabled`,
        400,
      );
    }
    ...
    if (
      hour >= config.peakHourSurcharge.startHour &&
      hour < config.peakHourSurcharge.endHour
    ) {
    ...
    if (input.codAmount > 0) {
      if (
        typeof config.codFee === "object" &&
        config.codFee.type === "percentage"
      ) {
        codFee = (input.codAmount * config.codFee.value) / 100;
      } else if (typeof config.codFee === "object") {
        codFee = config.codFee.value;
      } else {
        codFee = config.codFee; // legacy: plain number
      }
    }

    // Service multiplier
    const subtotal =
      (baseFare + distanceCharge + peakHourSurcharge + bulkyItemSurcharge) *
      service.multiplier;
