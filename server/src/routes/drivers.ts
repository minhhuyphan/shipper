...
router.patch('/:id/approve', validate(approveSchema), async (req: Request, res: Response) => {
    try {
        const driver = await driverService.approve(req.params.id, req.body.action);
        res.json(driver);
    } catch (error: any) {
        res.status(error.statusCode || 500).json({ error: error.message });
    }
...
router.post('/cod/settle', validate(settleSchema), async (req: AuthRequest, res: Response) => {
    try {
        const result = await driverService.settle(req.body, req.user._id.toString());
        res.json(result);
    } catch (error: any) {
        res.status(error.statusCode || 500).json({ error: error.message });
    }
...
router.get('/cod/export.csv', async (_req: Request, res: Response) => {
    try {
        ...
        res.setHeader('Content-Disposition', 'attachment; filename=cod-settlements.csv');
        res.send(csv);
    } catch (error: any) {
        res.status(error.statusCode || 500).json({ error: error.message });
    }
