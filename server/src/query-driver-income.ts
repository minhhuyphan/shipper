...
async function getDriverIncome() {
  try {
    await mongoose.connect(
      process.env.MONGO_URI || "mongodb://localhost:27017/shipper",
    );
    ...
    console.log(
      `💰 Tổng thu nhập: ${income.totalIncome.toLocaleString("vi-VN")}đ`,
    );
    console.log(
      `📈 Thu nhập trung bình/đơn: ${Math.round(income.avgIncome).toLocaleString("vi-VN")}đ`,
    );
    console.log(
      `📉 Thu nhập nhỏ nhất: ${income.minIncome.toLocaleString("vi-VN")}đ`,
    );
    console.log(
      `📈 Thu nhập lớn nhất: ${income.maxIncome.toLocaleString("vi-VN")}đ`,
    );
    console.log("\n");
    ...
    orders.forEach((order: any, idx: number) => {
      ...
      console.log(
        `   Service: ${order.serviceType} | Distance: ${order.distanceKm}km`,
      );
      console.log(
        `   Thu nhập: ${order.pricingBreakdown.total.toLocaleString("vi-VN")}đ`,
      );
      console.log(
